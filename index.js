  ...
  "node_modules/prompts/dist/prompts.js"(exports) {
    ...
    function toPrompt(type, args, opts = {}) {
      return new Promise((res, rej) => {
        const p3 = new el[type](args);
        const onAbort = opts.onAbort || noop;
        const onSubmit = opts.onSubmit || noop;
        const onExit = opts.onExit || noop;
        p3.on("state", args.onState || noop);
...
// src/prompts/create/handlers.ts
var cwd = process.cwd();
var makeTemplate = (result) => {
  var _a;
  const root = import_path3.default.join(cwd, result.targetDir);
  console.log(`
...
// src/prompts/create/index.ts
function create() {
  return (0, import_prompts.default)(steps).then(makeTemplate);
}
var templatesAsSelectOptions = TEMPLATES.flatMap((f) => f.variants).map(templateAsSelectOption);
var steps = [
  {
    type: "text",
    name: "targetDir",
    ...
    validate: (val) => import_fs3.default.existsSync(val) ? "Target directory already exists" ? !isValidPkgName(val) : "Not a valid package name, try a different one" : true
  },
  {
    type: "select",
    name: "template",
...
function start() {
  return (0, import_prompts2.default)(steps2).then((result) => result.command);
}
var options2 = [
  {
